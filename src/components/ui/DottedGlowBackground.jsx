 ...
 *
 * Ported from Aceternity UI (MIT) — adapted for vanilla CSS project.
 */
export default function DottedGlowBackground({
  ...
  useEffect(() => {
    ...
    // Precompute dot metadata and regenerate on resize
    let dots = [];
    ...
    const draw = (now) => {
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        // Fake the glow with a larger secondary circle instead of expensive shadowBlur
        if (a > 0.6) {
          const glow = (a - 0.6) / 0.4;
          ctx.globalAlpha = glow * opacity * 0.4;
          ctx.fillStyle = resolvedGlowColor;
          ctx.beginPath();
          ctx.arc(d.x, d.y, radius + (4 * glow), 0, Math.PI * 2);
          ctx.fill();
        }
      }
      ctx.restore();
